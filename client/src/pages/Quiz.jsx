  ...
  console.log("showMarks",showMarks)
	...
	return (
		<div className="max-w-[1300px] mx-auto">
          ...
          <div>
          You already attempted the test

          </div>
          <span className=" text-3xl text-center text-red-500">Your marks {user?.marks ? user?.marks :score} out of {quizData.length}</span>
							...
							<div className="flex flex-col ">
								...
								<div className=" flex flex-col items-start py-4 gap-y-8 ">
									{quizData[currQuestion]?.options?.map((elem, i) => (
										<button
											key={i}
											className={`p-1 border-none outline-none bg-slate-200 rounded-md w-full ${
												clickedOption === i + 1 ? "bg-green-400" : null
											} `}
